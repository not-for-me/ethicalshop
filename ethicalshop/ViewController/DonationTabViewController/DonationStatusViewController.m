    ...
    if([NetworkReachability connectedToNetwork]) {      
        ...
        [[StackMob stackmob] get:@"shop_info" withCallback:^(BOOL success, id result) {
            if(success) {
                ...
                if([UserObject userFileIsIn]) {
                    ...
                    [[StackMob stackmob] get:[NSString stringWithFormat:@"user/%@",[[UserObject sharedUserData] eMail]] withCallback:^(BOOL success, id result){
                        if(success) {                                
                            ...
                            [spinner stopAnimating];
                            [UIApplication sharedApplication].networkActivityIndicatorVisible = NO;                        
                            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"네트워크 오류" message:@"네트워크 장애로 사용자 데이터를 불러오지 못했습니다." delegate:self    cancelButtonTitle:@"확인" otherButtonTitles:nil];
                            [alert show];
                            [alert release];
                        }
                    }];
                ...
                [spinner stopAnimating];
                [UIApplication sharedApplication].networkActivityIndicatorVisible = NO;
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"네트워크 오류" message:@"네트워크 장애로 기부정보 데이터를 불러오지 못했습니다." delegate:self    cancelButtonTitle:@"확인" otherButtonTitles:nil];
                [alert show];
                [alert release];
            }
